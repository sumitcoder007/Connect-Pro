...
export declare class ZegoSuperBoardManager {
     ...
     */
    setCustomCursorAttribute(type: ZegoCursorType, attr: ZegoCursorAttribute): Promise<void>;
    /**
     * 设置白板工具类型
     *
     * 支持版本：2.0.0及以上。
    ...
    onWidgetPosition(x: number, y: number): void;
}
/**
...
export interface ZegoSuperBoardSubView {
     ...
     */
    switchSpeaker(label: string): Promise<MsgCode | boolean>;
    /**
     * 播放音视频
     *
     * 支持版本：2.12.0及以上。
     ...
     * 调用时机：当  currentSuperBoardSubView 存在的时候即可调用。
     *
     * @param id 音频、视频 HTML元素的 id 值
     * @param ct 视频进度值(可选)
     */
    playMedia(params: {
        id: string;
        ct?: number;
    }): Promise<MsgCode | boolean>;
    /**
     * 跳转到上一步动画
     *
     * 支持版本：2.0.0及以上。
...
export interface ZegoEvent {
    ...
    /**
     * Monitor the callback triggered when the passive end cannot play the audio and video due to browser restrictions, and return the id value of the audio and video html element If it is a video, it will return the progress of the video to be played together, which is generally used in conjunction with playMedia.
     */
...
declare type superBoardSubViewMediaPermissionCallback = (data: ZegoSuperBoardMediaModel) => void;
/**
 * 错误回调
 *
 */
...
export declare enum ZegoSuperboardCustomConfigKey {
     ...
     *
     * 使用示例：setCustomizedConfig('clearPageMode', '0')
     */
    ClearPageMode = "clearPageMode"
}
/**
 * 文件类型定义
 *
...
export interface ZegoSuperBoardCursorTitleStyle {
    title: string;
    bold: boolean;
    italic: boolean;
    size: number;
...
export declare enum ZegoSuperBoardCursorTitlePosition {
    leftTop = 0,
    rightTop = 1,
    rightBottom = 2,
    leftBottom = 3
}
export declare enum ZegoCursorType {
    pen = 1
}
/**
 * SuperBoard错误码枚举
...
export declare enum ZegoSuperBoardError {
     ...
     * PlayMedia Fail
     */
    ZegoSuperBoardErrorPlayMediaFail = 3130024,
    ZegoSuperBoardErrorDOCNotSupported = 3130025,
...
export interface ZegoSuperBoardMediaModel {
    id: string;
    type: string;
    fileType: ZegoSuperBoardFileType;
    ct: number;
